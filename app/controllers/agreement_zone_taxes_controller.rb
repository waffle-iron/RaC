...
class AgreementZoneTaxesController < ApplicationController
  ...
  def add_remove
    if @agreement_zone_tax
      ...
      redirect_to edit_agreement_zones_path(@agreement_zone.agreement_id, @agreement_zone.zone_id) , status: :see_other, notice: 'Se ha eliminado con exito el Impuesto de la Zona del Acuerdo.'
    else
      AgreementZoneTax.create({agreement_zone_id: @agreement_zone.id, tax_id: params[:tax_id]})
        redirect_to edit_agreement_zones_path(@agreement_zone.agreement_id, @agreement_zone.zone_id) , status: :see_other, notice: 'Se ha agregado con exito el Impuesto a la Zona del Acuerdo.'
    end
  end
