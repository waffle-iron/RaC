...
class AgreementZoneGroupsController < ApplicationController
  ...
  def add_remove_group
    if @agreement_zone_group
      ...
      redirect_to edit_agreement_zones_path(@agreement_zone.agreement_id, @agreement_zone.zone_id, anchor: :groups) , status: :see_other, notice: 'Se ha eliminado con exito el Grupo de la Zona del Acuerdo.'
    else
      AgreementZoneGroup.create({agreement_zone_id: @agreement_zone.id, group: params[:group_letter]})
        redirect_to edit_agreement_zones_path(@agreement_zone.agreement_id, @agreement_zone.zone_id, anchor: :groups) , status: :see_other, notice: 'Se ha agregado con exito el Grupo a la Zona del Acuerdo.'
    end
  end
