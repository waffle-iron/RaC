...
class AgreementZoneGroupSectionsController < ApplicationController
  ...
  def add_remove
    if @agreement_zone_group_section
      ...
      redirect_to edit_agreement_zones_path(@agreement_zone.agreement_id, @agreement_zone.zone_id, anchor: :group_sections) , status: :see_other, notice: 'Se ha eliminado con exito el Tramo de la Zona del Acuerdo.'
    else
      AgreementZoneGroupSection.create({agreement_zone_id: @agreement_zone.id, section: params[:section]})
        redirect_to edit_agreement_zones_path(@agreement_zone.agreement_id, @agreement_zone.zone_id, anchor: :group_sections) , status: :see_other, notice: 'Se ha agregado con exito el Tramo a la Zona del Acuerdo.'
    end
  end
